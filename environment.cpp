...
void cityBlock(pcl::visualization::PCLVisualizer::Ptr& viewer, ProcessPointClouds<pcl::PointXYZI>* pointProcessorI, const pcl::PointCloud<pcl::PointXYZI>::Ptr& inputCloud)
{
  ...
  //auto filterCloud = pointProcessorI->FilterCloud( inputCloud, 0.3f, Eigen::Vector4f(-(X / 2), -Y, -Z, 1), Eigen::Vector4f(X, Y, Z, 1));
    ...
    float filterRes = 0.15;  //change to 03->0.15
    Eigen::Vector4f minPoint(-10, -5, -2,1); //change to (-10, -5, -2,1)-->(-20, -6, -2, 1)
    Eigen::Vector4f maxPoint(30, 8, 1, 1); //change to (30, 8, 1, 1)-->( 30, 7, 5, 1)
    //ransac segment plane hyperparameters
    int maxIter = 50; //change to 25->50
    float distanceThreshold = 0.15; //change to 0.3->0.15
    // cluster hyperparameters
    float clusterTolerance = 0.25; //change to 0.5->0.25
    int minClusterSize = 10;
    int maxClusterSize = 500; //change to 1000
...
int main (int argc, char** argv)
{
  ...
  std::vector<boost::filesystem::path> stream = pointProcessorI->streamPcd("../src/sensors/data/pcd/data_1");
   ...
   //more challenge: src/sensors/data/pcd/data_2
